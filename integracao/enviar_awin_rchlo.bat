...
echo ============================================================
echo   EMISSAO EM PRODUCAO - AWIN Rchlo - competencia %COMPETENCIA% - valor %VALOR% - ordem %ORDEM% - SN %ALIQ%%%
echo ============================================================
...
python build_dps.py --fornecedor %FORNECEDOR% --competencia %COMPETENCIA% --valor %VALOR% --n-dps %NDPS% --tpAmb 1 --aliq-sn %ALIQ% --ordem %ORDEM%
if errorlevel 1 goto fim
python resumo_dps.py output\DPS_%FORNECEDOR%_%COMPETENCIA%.xml > output\resumo_producao_awin_rchlo.txt
type output\resumo_producao_awin_rchlo.txt
echo [3/4] Assinando...
python sign_dps.py output\DPS_%FORNECEDOR%_%COMPETENCIA%.xml secretos\raiana.pfx "%SENHA%"
if errorlevel 1 goto fim
echo [4/4] Enviando para PRODUCAO...
python submit_dps.py output\DPS_%FORNECEDOR%_%COMPETENCIA%_assinada.xml secretos\raiana.pfx "%SENHA%" --tpAmb 1 > output\resultado_producao_awin_rchlo.txt 2>&1
echo ---------------- RESPOSTA ----------------
type output\resultado_producao_awin_rchlo.txt
echo ------------------------------------------
:fim
set SENHA=
echo.
