...
echo ============================================================
echo   EMISSAO EM PRODUCAO - Magalu - competencia %COMPETENCIA% - valor %VALOR% - SN %ALIQ%%%
echo ============================================================
...
python resumo_dps.py output\DPS_%FORNECEDOR%_%COMPETENCIA%.xml > output\resumo_producao_magalu.txt
type output\resumo_producao_magalu.txt
echo [3/4] Assinando...
python sign_dps.py output\DPS_%FORNECEDOR%_%COMPETENCIA%.xml secretos\raiana.pfx "%SENHA%"
if errorlevel 1 goto fim
echo [4/4] Enviando para PRODUCAO...
python submit_dps.py output\DPS_%FORNECEDOR%_%COMPETENCIA%_assinada.xml secretos\raiana.pfx "%SENHA%" --tpAmb 1 > output\resultado_producao_magalu.txt 2>&1
echo ---------------- RESPOSTA ----------------
type output\resultado_producao_magalu.txt
echo ------------------------------------------
:fim
set SENHA=
echo.
