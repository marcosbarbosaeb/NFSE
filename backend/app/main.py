...
// de propósito (ex.: HTTPException), mas como uma LISTA de objetos quando é o
// Pydantic validando o corpo automaticamente (422 de campo obrigatório/formato
// errado) — sem isso, essas mensagens apareciam como "[object Object]" pro
// usuário, o que parecia "não funciona" mesmo quando o erro era só, por
// exemplo, competência em formato errado.
...
async function gerar() {
  ...
  if (!vinculoId) { div.innerHTML = '<p class="erro">Escolha um vínculo (fornecedor) na lista antes de gerar.</p>'; return; }
  if (!competencia) { div.innerHTML = '<p class="erro">Escolha o mês de competência.</p>'; return; }
